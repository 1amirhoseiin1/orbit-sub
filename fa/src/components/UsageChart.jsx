...
export default function UsageChart({ usedGB, totalGB }) {
  ...
  // animate the ring from 0 on mount instead of snapping to value
      ...
      <Box sx={{ position: 'relative', display: 'inline-flex', flexShrink: 0 }}>
        ...
        >
          ...
          <Typography variant="caption" sx={{ color: 'rgba(250,250,250,0.5)' }}>
            {unlimited ? 'نامحدود' : 'مصرف‌شده'}
          </Typography>
        </Box>
      </Box>

      <div className="flex flex-col gap-3 w-full">
        <Note color="#3B82F6" label="حجم مصرف‌شده" value={`${usedGB.toFixed(1)} گیگابایت`} />
        <Note
          color="#06B6D4"
          label="حجم باقی‌مانده"
          value={unlimited ? 'نامحدود' : `${remaining.toFixed(1)} گیگابایت`}
        />
        <Note color="#8B5CF6" label="حجم کل" value={unlimited ? 'نامحدود' : `${totalGB.toFixed(1)} گیگابایت`} />
      </div>
    </GlowCard>
  );
}
