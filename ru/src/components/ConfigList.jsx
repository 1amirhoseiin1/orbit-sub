...
function pluralServers(n) {
  const mod10 = n % 10;
  const mod100 = n % 100;
  if (mod100 >= 11 && mod100 <= 14) return 'серверов';
      ...
      <div className="flex items-center justify-between mb-4 gap-3">
        <div className="flex items-center gap-2">
          ...
          <Typography variant="caption" sx={{ color: 'rgba(250,250,250,0.5)' }}>
            {configs.length} {pluralServers(configs.length)}
          </Typography>
        </div>

        <IconActionButton
          icon={copiedAll ? <CheckRoundedIcon /> : <ContentCopyRoundedIcon />}
          tooltip={copiedAll ? 'Всё скопировано' : 'Копировать все конфигурации'}
          tint="blue"
          success={copiedAll}
          onClick={handleCopyAll}
          ariaLabel="Копировать все конфигурации"
        />
      </div>
